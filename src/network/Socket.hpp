...
public:
    ...
    {
        if (udp_server(host, serv, nullptr) < 0)
        {
...
private:
    ...
    int udp_client(const char *host, const char *serv)
    {
        ...
        if ((n = getaddrinfo(host, serv, &hints, &res)) != 0)
            Throw("udp_client error for %s, %s: %s", host, serv, gai_strerror(n));
        ressave = res;
        ...
        if (res == NULL) /* errno set from final socket() */
            Throw("udp_client error for %s, %s", host, serv);
